...
const AppBuildingCSH = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const studentUnionDocRef = doc(db, "한동대학교", "코너스톤홀");

        const floorsData = [];
        const collections = ["1층"]; // 필요한 하위 컬렉션 이름을 여기에 추가합니다.
